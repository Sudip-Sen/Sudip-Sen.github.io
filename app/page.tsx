...
const socials = [
  ...
  { name: "GitHub", url: "https://github.com/Sudip-Sen", logo: "https://upload.wikimedia.org/wikipedia/commons/9/91/Octicons-mark-github.svg", invert: true },
...
export default function Home() {
  ...
  const styles: Record<string, React.CSSProperties> = {
    nav: { position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: isDark ? "rgba(10,15,13,0.85)" : "rgba(245,249,246,0.85)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--card-border)", padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: "64px" },
    logo: { fontFamily: "'DM Serif Display', serif", fontSize: "1.2rem", color: "var(--foreground)" },
    section: { padding: "5rem 2rem", position: "relative", zIndex: 1 },
    sectionAlt: { padding: "5rem 2rem", background: isDark ? "rgba(15,23,18,0.7)" : "rgba(237,244,239,0.7)", position: "relative", zIndex: 1 },
    container: { maxWidth: "1100px", margin: "0 auto" },
    card: { background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: "12px", padding: "1.5rem", marginBottom: "1.2rem", transition: "border-color 0.3s, transform 0.3s" },
  };
  ...
  return (
    <>
      ...
      <nav style={styles.nav}>
        <div style={styles.logo}>Sudip <span style={{ color: accent }}>Sen</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
          <div style={{ display: menuOpen ? "flex" : "none", position: "fixed", top: "64px", left: 0, right: 0, background: isDark ? "rgba(10,15,13,0.95)" : "rgba(245,249,246,0.95)", flexDirection: "column", padding: "2rem", gap: "1.5rem", borderBottom: "1px solid var(--card-border)", zIndex: 99 }}>
            {["about","education","publications","experience","skills","contact"].map(s => (
              <a key={s} href={"#"+s} onClick={() => setMenuOpen(false)} style={{ color: "var(--text2)", textTransform: "uppercase", fontSize: "0.85rem", letterSpacing: "0.05em", textDecoration: "none" }}>{s}</a>
            ))}
          </div>
          <div style={{ display: "flex", gap: "2rem", alignItems: "center" }}>
            {["about","education","publications","experience","skills","contact"].map(s => (
              <a key={s} href={"#"+s} className="nav-desktop-link" style={{ color: "var(--text2)", textTransform: "uppercase", fontSize: "0.8rem", letterSpacing: "0.05em", textDecoration: "none", display: "none" }}>{s}</a>
            ))}
          </div>
          <button onClick={() => setTheme(isDark ? "light" : "dark")} style={{ background: "var(--card)", border: "1px solid var(--card-border)", color: "var(--foreground)", width: "38px", height: "38px", borderRadius: "50%", cursor: "pointer", fontSize: "1rem" }}>
            {isDark ? "🌙" : "☀️"}
          </button>
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--foreground)", fontSize: "1.5rem" }}>☰</button>
        </div>
      </nav>
      ...
      <section id="home" style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "6rem 2rem 4rem", position: "relative", zIndex: 1 }}>
        <div style={styles.container}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "var(--accent-dim)", border: "1px solid var(--accent)", color: accent, fontSize: "0.75rem", fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", padding: "0.4rem 1rem", borderRadius: "100px", marginBottom: "1.5rem" }}>
            ...
            {socials.map((s) => (
              <a
                ...
                style={{
                  ...
                  height: "48px",
                  borderRadius: "50%",
                  background: hoveredSocial === s.name ? accent : "var(--accent-dim)",
                  border: "2px solid var(--accent)",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  textDecoration: "none",
                  transform: hoveredSocial === s.name ? "translateY(-4px)" : "translateY(0)",
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginTop: "1.5rem" }}>
                {["🌾 Precision Agriculture","🛰️ Remote Sensing","🤖 Machine Learning","🌱 Crop System Modelling","🚁 UAV / Drone","🗺️ GIS","🐍 Python","📊 R Programming"].map(tag => (
                  <span key={tag} style={{ background: "var(--accent-dim)", color: accent, border: "1px solid var(--accent)", padding: "0.35rem 0.85rem", borderRadius: "100px", fontSize: "0.8rem", fontWeight: 500 }}>{tag}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "2rem" }}>
            {[["all","All (6)"],["published","Published"],["review","Under Review"],["book","Book Chapter"]].map(([val, label]) => (
              <button key={val} onClick={() => setActiveFilter(val)}
                style={{ background: activeFilter === val ? accent : "var(--card)", color: activeFilter === val ? "#000" : "var(--text2)", border: "1px solid var(--card-border)", padding: "0.4rem 1rem", borderRadius: "100px", fontSize: "0.8rem", cursor: "pointer" }}>
                {label}
              </button>
            ))}
          </div>
            ...
            {filtered.map((pub, i) => (
              <div key={i} style={{ ...styles.card, marginBottom: 0 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", marginBottom: "0.75rem" }}>
                  <span style={{ background: "var(--accent-dim)", color: accent, border: "1px solid var(--accent)", padding: "0.25rem 0.7rem", borderRadius: "100px", fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", whiteSpace: "nowrap" }}>
            ...
            {[
              ...
              <div key={i} style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: "12px", padding: "1.5rem" }}>
                ...
                {group.skills.map(([name, pct]) => (
                  <div key={String(name)} style={{ marginBottom: "0.75rem" }}>
                    ...
                    <div style={{ height: "4px", background: "var(--line)", borderRadius: "2px" }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: accent, borderRadius: "2px" }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              {[
                ...
                <div key={i} style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: "12px", padding: "1.25rem", display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1rem" }}>
                  <div style={{ width: "44px", height: "44px", background: "var(--accent-dim)", border: "1px solid var(--accent)", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", flexShrink: 0 }}>{item.icon}</div>
                  <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  {[["Name","name","Your name"],["Email","email","your@email.com"]].map(([label, name, ph]) => (
                    <div key={String(name)}>
                      ...
                      <input name={String(name)} placeholder={String(ph)} style={{ background: "var(--card)", border: "1px solid var(--card-border)", color: "var(--foreground)", padding: "0.75rem 1rem", borderRadius: "8px", fontFamily: "'DM Sans',sans-serif", fontSize: "0.9rem", width: "100%", outline: "none" }} />
                    </div>
                  ))}
                </div>
                <div>
      ...
      <style>{`
        ...
        @media(min-width:768px){ .nav-desktop-link { display: flex !important; } }
      `}</style>
    </>
  );
}
